...
function Invoke-Make([string] $makeCmd, [string] $makeFlags, [string] $sdkPath, [string] $modSrcRoot) {
    ...
    $messageData = New-Object psobject -property @{
        ...
        # for resolving the path names back, we just store them in a hash table the first time we resolved them
        # since it's faster than using any sort of I/O access
        paths = @{}
    }
    ...
    $outAction = {
        ...
        if (($outTxt -Match "Error|Warning") -And ($outTxt -Match $messagePattern)) {
            ...
            # We need to resolve the file back -- we may have copied it from a subdirectory into the Classes folder, so we need to find the file again
            # For now, we just search the file name -- this is slow, but (since we are organizing it into subdirectories) not *as* slow
            # Additionally, we format it from `filepath(line) : message` to `filepath:line : message` to make the jump-to-error functionality in VS Code work
            $fName = $replacementTxt -Replace $messagePattern, '$2'
            [Hashtable]$files = $event.MessageData.paths
            if (!$files.ContainsKey($fName)) {
                $file = Get-ChildItem -Path $fileBasePath -Filter $fName -Recurse
                $files.Add($fName, $file.FullName)
            }
            $actualName = $files.Get_Item($fName)
            $appendix = $replacementTxt -Replace $messagePattern, ':$3 : $4'

            $outTxt = "$actualName$appendix"
        }
...
# we allow mods to add their files to subdirectories of Classes to organize them better
# for each package's classes folder, for each folder in there, get recursively all its files and move it to classes. afterwards, delete the folders
Write-Host "Moving subdirectory contents to Classes"
for ($i=0; $i -lt $allpackages.length; $i++) {
    $packagename = $allpackages[$i]
    Get-ChildItem -Path "$sdkPath/Development/Src/$packagename/Classes/" -Recurse -Directory | Get-ChildItem -Recurse -File | Move-Item -Destination "$sdkPath/Development/Src/$packagename/Classes/"
    Get-ChildItem -Path "$sdkPath/Development/Src/$packagename/Classes/" -Recurse -Directory | Remove-Item -Recurse
}
...
# For now, just comment it out -- the two or three custom materials can be compiled at runtime.
<#
